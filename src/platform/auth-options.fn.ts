 ...
 *   src/platform/server/auth/config.ts). Always false in production builds, where
 *   `import.meta.env.DEV` is define-replaced — this discloses nothing.
 */

import { isDevFixedOtpActive } from '@/platform/server/auth/config';
import { isGoogleAuthConfigured } from '@/platform/server/auth/google-auth';
